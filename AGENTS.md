...
- 当前 session 调 `.agents/skills/pr/scripts/review_with_opus.sh`，由 Teamorouter 路由到 `claude-opus-5`，**默认 `--effort medium`**（可用 `REVIEW_EFFORT` 覆盖为 low/medium/high/xhigh/max，非法值直接报错退出），做无工具、无会话持久化的只读 diff review。该脚本要求结构化 JSON-Schema 输出，由 `render_review.py` 确定性地渲染成评论正文并本地推导结论；非法响应最多重试 3 次，每次调用有 `REVIEW_TIMEOUT_SEC`（默认 1800s）的 wall-clock 上限。**effort 默认值的实测依据**（约 1300 行 diff 的 PR #48）：`low` 约 450s 返回但 findings 为空（同一 SHA 的 codex reviewer 找到 2 个真 blocker）；`high` 跑满两次 900s 仍未产出 `structured_output`，只会烧完重试后失败；`medium` 超过 1200s。所以默认取 `medium` 并把上限放到 1800s
