...
# Render source: substitute placeholder with absolute launcher path.
tmp_src=$(mktemp -t mycalfix.XXXXXX).applescript
trap 'rm -f "$tmp_src"' EXIT
sed -e "s|__LAUNCHER_PATH__|$LAUNCHER|g" "$SRC_APPLESCRIPT" > "$tmp_src"
...
echo
echo "tail logs:  tail -f $ROOT/logs/launch_fix.log"
